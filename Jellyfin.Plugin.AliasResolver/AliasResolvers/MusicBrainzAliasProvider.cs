    ...
    IHasOrder, IForcedProvider, IDisposable, IHasItemChangeMonitor
{
    ...
    public bool HasChanged(BaseItem item, IDirectoryService directoryService)
    {
        if (item is Audio audio)
        {
            ...
            {
                return MusicBrainzAliasResolver.GetItemStatus(item) == MusicBrainzAliasStatus.Unknown;
            }
        }
    ...
    private async Task<ItemUpdateType> ResolveAlbumAlias(MusicAlbum item, MetadataRefreshOptions options, CancellationToken cancellationToken)
    {
        ...
        var status = MusicBrainzAliasResolver.GetItemStatus(item);
        var finalStatus = status;
        var updateValue = ItemUpdateType.None;

        if (Plugin.Instance!.Configuration.DoAlbum && !string.IsNullOrEmpty(releaseId) && !string.IsNullOrEmpty(releaseGroupId) && (status == MusicBrainzAliasStatus.Unknown || options.RemoveOldMetadata))
        {
            _logger.LogDebug("Album ids: {Release}:{ReleaseGroup}", releaseId, releaseGroupId);
            ...
            {
                item.TrySetProviderId(ProviderKeys.MusicBrainzAlias.ToString(), finalStatus.ToString());
                updateValue |= ItemUpdateType.MetadataEdit;
            }
        }

        if (Plugin.Instance!.Configuration.DoArtist && item.AlbumArtist != item.MusicArtist.Name && !string.IsNullOrEmpty(item.MusicArtist.Name))
        {
    ...
    private async Task<ItemUpdateType> ResolveAudioAlias(Audio item, MetadataRefreshOptions options, CancellationToken cancellationToken)
    {
        ...
        _logger.LogDebug("Recording id: {Id}:{ReleaseId}", recordingId, releaseId);

        if (Plugin.Instance!.Configuration.DoTrack && !string.IsNullOrEmpty(recordingId) && !string.IsNullOrEmpty(releaseId) && (status == MusicBrainzAliasStatus.Unknown || options.RemoveOldMetadata))
        {
            MusicBrainzAliasResolver aliasResolver = new MusicBrainzAliasResolver(item.Name, _logger);
            aliasResolver.SetRatioLimit(Plugin.Instance!.Configuration.Threshold);
            if (aliasResolver.ShouldUpdate() || options.RemoveOldMetadata)
        ...
        if (Plugin.Instance!.Configuration.DoAlbum && item.AlbumEntity.Name != item.Album)
        {
            item.Album = item.AlbumEntity.Name;
            updateValue |= ItemUpdateType.MetadataEdit;
        }

        if (Plugin.Instance!.Configuration.DoArtist && !item.AlbumArtists.Contains(item.AlbumEntity.AlbumArtist) && !string.IsNullOrEmpty(item.AlbumEntity.AlbumArtist))
        {
    ...
    private async Task<ItemUpdateType> ResolveArtistAlias(MusicArtist item, MetadataRefreshOptions options, CancellationToken cancellationToken)
    {
        ...
        if (Plugin.Instance!.Configuration.DoArtist && !string.IsNullOrEmpty(artistId))
        {
            if (status == MusicBrainzAliasStatus.Unknown || options.RemoveOldMetadata)
            {
